...
class KalmanNetNN(torch.nn.Module):
    ...
    def KNet_step(self, y):
        ...
        KNet_out_m1x_posterior.detach()
        # adding constraint on the Knet output to be within a desired range
        # if not in the range - takes the closest value within 
        # the problem's acceptable range
        if self.args.is_robot_case:
            self.m1x_posterior = make_state_valid(KNet_out_m1x_posterior)
        else: 
            self.m1x_posterior = KNet_out_m1x_posterior
        ...
        # return
        return self.m1x_posterior # if state else return dy
    ...
    # def change_2_valid_posterior(self):
    #     if not(is_valid(self.m1x_posterior)
    #            self.m1x_posterior = get_valid_state(self.m1x_posterior)
