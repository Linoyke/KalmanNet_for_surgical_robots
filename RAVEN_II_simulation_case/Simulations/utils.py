...
def Short_Traj_Split(data_target, data_input, T):
   ...
   """ Split data along the first dimension into sequences of length T+1.
    
    Args:
        data_target (torch.Tensor): The target data tensor.
        data_input (torch.Tensor): The input data tensor.
        T (int): The desired sequence length for each split.
        
    Returns:
        list: A list containing split targets, inputs, and initial states."""

    # Check if there is enough data to split
   if data_target.size(0) > T:
        data_target = list(torch.split(data_target, T+1, 0))
        data_input = list(torch.split(data_input, T+1, 0))

        # Check and remove the last chunk if it does not fulfill the length requirement
        if data_target[-1].size(0) != T+1:
            data_target.pop()
        if data_input[-1].size(0) != T+1:
            data_input.pop()
   else:
        raise ValueError("Not enough data to form complete chunks of size T+1")

    # Concatenate the list back into a tensor
   data_target = torch.cat(data_target, dim=0)
   data_input = torch.cat(data_input, dim=0)

    # Separate the initial state
   init = data_target[:, :, 0]
